...
{
    public class PrefixSubQueryExamineTranslator: ISubQueryExamineTranslator<PrefixSubQuery>
    {
        public Query Translate<TViewModel>(ISearcher searcher,IEnumerable<ISubQueryElasticTranslator> collection, ISearchSubQuery query) where TViewModel : class
        {
            ...
            var termSubQuery = (PrefixSubQuery)query;
            if (termSubQuery.Value == null || termSubQuery.Field == null)
            {
                return  null;
            }
            nestedQuery.Field(termSubQuery.Field,termSubQuery.Value.ToString().MultipleCharacterWildcard());
